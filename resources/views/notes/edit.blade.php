    ...
    <div class="col-md-8">
        <div class="card shadow-sm">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 text-primary"><i class="bi bi-pencil-square me-2"></i>Modification de Note</h5>
                ...
                <form action="{{ route('notes.update', $note) }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="note" class="form-label fw-bold">Note / 20</label>
                            <input type="number" step="0.25" min="0" max="20" 
                                   class="form-control @error('note') is-invalid @enderror" 
                                   id="note" name="note" value="{{ old('note', $note->note) }}" required>
                            @error('note') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="type_devoir" class="form-label fw-bold">Type d'Évaluation</label>
                    ...
                    <div class="d-flex justify-content-between mt-4">
                        ...
                        <button type="submit" class="btn btn-primary px-4">
                            <i class="bi bi-save me-1"></i>Mettre à jour la Note
                        </button>
                    </div>
                </form>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentMatiereId = "{{ $note->matiere_id }}";

    function filterMatieres(preserveValue = null) {
        const selectedEleve = eleveSelect.options[eleveSelect.selectedIndex];
        const classeId = selectedEleve ? selectedEleve.dataset.classe : null;
        ...
        if (classeId) {
            matiereOptions.forEach(opt => {
                if (opt.dataset.classe === classeId) {
                    ...
                    if (preserveValue && newOpt.value == preserveValue) {
                        newOpt.selected = true;
                    }
                    matiereSelect.appendChild(newOpt);
                }
            });
        }
    }
    ...
    // Initialisation
    filterMatieres(currentMatiereId);
});
</script>
@endsection
