...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="mb-0">Modifier l'année académique</h2>
    ...
    <div class="card-body">
        <form action="{{ route('annees.update', $annee) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="libelle" id="libelle" class="form-control" value="{{ old('libelle', $annee->libelle) }}" required>
                @error('libelle') <div class="text-danger mt-1">{{ $message }}</div> @enderror
            </div>

            <div class="form-check mb-3">
                <input class="form-check-input" type="checkbox" value="1" id="active" name="active" {{ $annee->active ? 'checked' : '' }}>
