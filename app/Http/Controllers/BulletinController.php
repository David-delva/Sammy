...
class BulletinController extends Controller
{
    ...
    public function show(Eleve $eleve)
    {
        ...
        return view('bulletins.pdf', compact('eleve', 'annee', 'classe', 'results', 'moyenneGenerale', 'rang', 'totalEleves'));
    }
    ...
    {
        $annee = currentAcademicYear();
        if (!$annee) return back()->with('error', 'Aucune année académique active.');

        $inscription = $eleve->inscriptions()->where('annee_academique_id', $annee->id)->first();
        if (!$inscription) return back()->with('error', "Pas d'inscription trouvée.");
